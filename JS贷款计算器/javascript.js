...
function getLeaders(amount,apr,years,zipcode){
	if(!window.XMLHttpRequest) return;//如果浏览器不支持XMLHttpRequest对象，则退出
	var ad = document.getElementById("lenders");//找到要显示放贷人列表的元素
	...
	req.onreadystatechange = function(){
		if(req.readyState === 4 && req.status === 200){
			...
			for(var i=0;i<leaders.length;i++){
				list += "<li><a href='" + leaders[i].url + "'>" +leaders[i].name +"</a>";
			}
...
function chart(principal,interest,monthly,payments){
	...
	for(var p=1;p<=payments;p++){
		var thisMonthsInterest = (principal - equaity) * interest;
	...
	g.textAlign = "center";
	var y = amountToY(0);
	for(var year=1;year*12<=payments;years++){
